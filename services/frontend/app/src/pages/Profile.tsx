...
const Profile = () => {
    ...
    const handleClaimReward = async (achievementId: number) => {
        try {
            // Llamada al método claimAchievement del userService
            const response = await userService.claimAchievement(achievementId);

            if (response) {
                // Actualización inmutable del estado para reflejar el cambio visualmente
                setProfileData(prev => {
                    if (!prev) return prev;
                    ...
                    // También deberíamos actualizar los puntos en el estado si el backend los devuelve
    ...
    const formattedHistory = profileData?.match_history?.map(match => {
        ...
        return { ...match, resultString, translatedResult, opponentName, opponentAvatar, scoreFormatted, dateFormatted, styles };
    }) || [];
