...
import { FaTrophy, FaSearch, FaMinus } from "react-icons/fa";
...
const Ranking = () => {
	...
	const [rankingData, setRankingData] = useState<UserProfile[]>([]);
		...
		<DashboardLayout isCentered={false}>
			<div className="max-w-5xl mx-auto w-full animate-fade-in-up pb-24">
				...
				<div className="flex flex-col md:flex-row justify-center items-center md:items-end gap-1 md:gap-4 mb-12 w-full">
					{/* 1ST PLACE */}
					<div className="order-1 md:order-2 w-full md:w-1/3 flex justify-center z-10 hover:scale-105 transition-transform">
						{topThree[0] && (
							<Link to={`/profile/${topThree[0].id}`} className="w-full">
								<PodiumCard player={topThree[0]} place={1} delay="0ms" isWinner />
							</Link>)}
					</div>

					{/* 2ND PLACE */}
					<div className="order-2 md:order-1 w-full md:w-1/3 flex justify-center z-10 hover:scale-105 transition-transform">
						{topThree[1] && (
							<Link to={`/profile/${topThree[1].id}`} className="w-full">
								<PodiumCard player={topThree[1]} place={2} delay="100ms" />
							</Link>
						)}
					</div>

					{/* 3RD PLACE */}
					<div className="order-3 md:order-3 w-full md:w-1/3 flex justify-center z-10 hover:scale-105 transition-transform">
						{topThree[2] && (
							<Link to={`/profile/${topThree[2].id}`} className="w-full">
								<PodiumCard player={topThree[2]} place={3} delay="200ms" />
							</Link>
						)}
					</div>
				</div>
							...
							<div key={player.id} className={`glass-panel p-4 relative overflow-hidden ${isCurrentUser ? "border-brand-500/50 bg-brand-500/5" : ""}`}>
								...
								<div className="flex items-center gap-3 mb-3">
									...
									<div className="flex-1">
										<Link to={`/profile/${player.id}`} className="block hover:opacity-80 transition-opacity cursor-pointer">
											<PlayerBadge
								...
								return (
									<tr key={player.id} className={`hover:bg-white/5 transition-colors group ${isCurrentUser ? "bg-brand-500/10" : ""}`}>
										...
										<td className="px-6 py-4">
											<Link to={`/profile/${player.id}`} className="inline-block w-full hover:opacity-80 transition-opacity cursor-pointer">
												<PlayerBadge
													avatar={player.avatar ? player.avatar : undefined}
													name={player.username}
													isCurrentUser={isCurrentUser}
